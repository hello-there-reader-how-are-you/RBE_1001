...
def detect_fruits():
    ...
    if all_fruits:
        all_fruits.sort(key=lambda basket: basket.height, reverse=True) # Sorts fruit by hieght, analogus to distance
    return all_fruits

def detect_baskets(id):
...
def home_arm():
    ...
    arm_motor.set_timeout(4, SECONDS)
    arm_motor.spin_for(REVERSE, 4, TURNS, True)
    arm_motor.stop()
    arm_motor.set_max_torque(100, PERCENT)

def Approach_Fruit(*args):
    print("Approach_Fruit")
    imu.set_heading(0)
...
def Pick_Fruit(found_fruit):
    ...
    while abs(scroll(imu.heading()) - og_angle) <= 10: # Magic Number
    ...
    arm_motor.spin_for(FORWARD, 0.4, TURNS, True)
...
def Drive_To_Basket(id):
    ...
    while True:
        ...
        if (basket):
            ...
            print("B:", basket.height)

            if basket.height >= 1000000000:
                arm_motor.stop()
                left_motor.stop()
                right_motor.stop()
...
home_arm()
print("Arm Homed")
arm_motor.spin_for(FORWARD, 2, TURNS, True)
